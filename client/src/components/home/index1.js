...
const styles = {
  ...
  '.knob-surround': {
    ...
    margin: '5em auto',
    background: '#181818 -webkit-gradient(linear, left bottom, left top, color-stop(0, #1d1d1d), color-stop(1, #131313)) -ms-linear-gradient(bottom, #1d1d1d, #131313) -moz-linear-gradient(center bottom, #1d1d1d 0%, #131313 100%) -o-linear-gradient(#131313, #1d1d1d)',
...
function createTicks(numTicks, highlightNumTicks) {
  ...
  for(var i=0;i<numTicks;i++) {
      ...
      tickLabel.innerHTML = 'Prev';
      tickLabel.style.className = { ...styles['.min'] };

      //highlight only the appropriate ticks using dynamic CSS
      if(i < highlightNumTicks) {
          tick.className = 'activetick';
      } else {
          tick.className = 'tick';
      }
      ...
      startingTickAngle += 60;
  }

  startingTickAngle = -115; //reset
}
...
function onMouseMove(event) {
  ...
  finalAngleInDegrees = -(getRadiansInDegrees + 115); //knob is already starting at -135 degrees due to visual design so 135 degrees needs to be subtracted to compensate for the angle offset, negative value represents clockwise direction

  console.log("finalAngleInDegrees: ", finalAngleInDegrees);
   

  //only allow rotate if greater than zero degrees or lesser than 270 degrees
  if(finalAngleInDegrees >= -240 && finalAngleInDegrees <= 2) {
      volumeKnob.style.transform = "rotate(" + finalAngleInDegrees + "deg)"; //use dynamic CSS transform to rotate volume knob

      showModal({ handleClose: ()=>{}, show: false });
      if(finalAngleInDegrees >= 0 && finalAngleInDegrees <=5) {
        showModal({ handleClose: ()=>{}, show: true, location: 'current' });
      } else if(finalAngleInDegrees >= -65 && finalAngleInDegrees <=-45) {
        showModal({ handleClose: ()=>{}, show: true, location: 'prev1' });
      }

      //270 degrees maximum freedom of rotation / 100% volume = 1% of volume difference per 2.7 degrees of rotation
      // volumeSetting = Math.floor(finalAngleInDegrees / (50 / 100));

      // tickHighlightPosition = Math.round((volumeSetting * 2.7) / 10); //interpolate how many ticks need to be highlighted

      // createTicks(5, tickHighlightPosition); //highlight ticks
...
const showModal = ({ handleClose, show, children=undefined, location='current' }) => {
  if(!show) {
    const currentPopup = document.getElementById("myPopup");
  ...
  let content;
  if(location === 'current') {
    modalPositionStyle = "top: 160px; left: 190px;";
    content = '<span class="popuptext" id="myPopup">Company Name: Advance Auto Parts <br/> Date <br/> Technologies: React, Redux, Express, Node, Typescript, HTML, Javascript, CSS, Mongo, DB2 </span>';
  } else if(location === 'prev1') {
    modalPositionStyle = "top: 110px; left: 210px;";
    content = '<span class="popuptext" id="myPopup">Company Name: BlueSky Technology <br/> Date <br/> Technologies: React, Redux, Express, Node, Typescript, HTML, Javascript, CSS, Mongo, DB2 </span>';
  }
  const tickContainer = document.getElementById("tickContainer");
  const divElement = document.createElement("div");
  
  divElement.className = "popup";
  divElement.style.cssText = modalPositionStyle;
  divElement.innerHTML = content;
  
  tickContainer.appendChild(divElement);
...
const Home = () => {
  useEffect(() => {
    ...
    createTicks(5, 0);
    showModal({ handleClose: ()=>{}, show: true, children: <div>ytest</div> });
    return () => {
  ...
  return (
    <Fragment>
      <div style={styles['.card']}>
          <h1>Sudhin Justin</h1>
            <p style={styles['.title']}>President & Founder, BlankSpace Consulting Inc.</p>
            <p style={styles['.p']}>Ecommerce Consultant</p>
            <p style={styles['.p']}>
              <a href="/demo">Demo Project</a>
            </p>
            <p>
              <div style={styles['.knob-surround']}>
                <div id="knob" className='knob'></div>
                <span style={styles['.min']}>Min</span>
                <span style={styles['.prev1']}>Prev</span>
                <span style={styles['.max']} id="currentNode">Current</span>
                <div id="tickContainer" style={styles['.tick']}></div>
              </div>
            </p>
